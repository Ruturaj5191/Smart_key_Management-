  ...
  const loadData = async () => {
    ...
    try {
      const [histRes, billRes] = await Promise.all([
        api.get("/owner/water-history"),
        api.get("/owner/water-bill")
      ]);
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Total Units/Bottles</p>
        ...
        <div className="rounded-2xl border border-blue-100 bg-blue-50/30 p-5 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wider text-blue-600">Total Water Bill</p>
          <p className="mt-2 text-3xl font-bold text-blue-700">₹{bill.total_amount || 0}</p>
        </div>
      </div>

      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
          <h3 className="text-lg font-semibold text-slate-900">Water Request History</h3>
          <button 
            onClick={loadData}
            ...
            <tbody className="divide-y divide-slate-100 text-slate-900">
              {history.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-medium">{r.quantity} Units</td>
