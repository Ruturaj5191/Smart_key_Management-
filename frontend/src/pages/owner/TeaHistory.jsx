...
export default function TeaHistory() {
  ...
  const loadData = async () => {
    ...
    try {
      const [histRes, billRes] = await Promise.all([
        api.get("/owner/tea-history"),
        api.get("/owner/tea-bill")
      ]);
      setHistory(histRes.data.data || []);
      setBill(billRes.data.data || { total_requests: 0, total_quantity: 0, total_amount: 0 });
    } catch (err) {
      console.error("Failed to load tea data");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Total Cups</p>
          <p className="mt-2 text-3xl font-bold text-slate-900">{bill.total_quantity || 0}</p>
        </div>
        <div className="rounded-2xl border border-brand-100 bg-brand-50/30 p-5 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wider text-brand-600">Total Monthly Bill</p>
          <p className="mt-2 text-3xl font-bold text-brand-700">₹{bill.total_amount || 0}</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
          <h3 className="text-lg font-semibold text-slate-900">Tea Request History</h3>
          <button 
            onClick={loadData}
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50 text-slate-600">
              <tr>
                ...
                <th className="px-6 py-3 text-left font-semibold uppercase tracking-wider">Qty</th>
            ...
            <tbody className="divide-y divide-slate-100 text-slate-900">
              {history.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-medium">{r.quantity} Cups</td>
              ...
              {!history.length && !loading && (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                    ...
                    No tea requests found. Time for some tea?
                  </td>
                </tr>
              )}
            </tbody>
